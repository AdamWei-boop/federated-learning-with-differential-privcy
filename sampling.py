...
def mi_mnist_iid(args, dataset, num_users, num_items, frac):
    """
    ...
    """       
    dict_train, dict_train_shadow, all_idxs = {}, {}, [i for i in range(len(dataset))]     
    for i in range(num_users):  
        dict_train[i] = set(np.random.choice(all_idxs, int(num_items*(1-frac)), replace=False))
        all_idxs = list(set(all_idxs) - dict_train[i])
        dict_train_shadow[i] = set(np.random.choice(all_idxs, int(num_items*frac), replace=False))
        all_idxs = list(set(all_idxs) - dict_train_shadow[i])
                  
    return dict_train, dict_train_shadow
